...
const { clientss } = require('./instagram'); 
...
app.get('/check-user-instagram/:instagramId', async (req, res) => {
    ...
    if (clientss[instagramId] && clientss[instagramId].connected) {
        return res.json({ connected: true, username: clientss[instagramId].username || "Bilinmeyen Kullanıcı" });
    }
